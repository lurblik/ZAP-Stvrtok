...
/*
* Params:
* argc - argument count, pocet vstupnych argumentov
* argv - hodnoty vstupnych argumentov -> na prvom mieste
*       sa vzdy nachadza nazov spusteneho suboru, po nom nasleduju
*       dalsie argumenty z prikazoveho riadka
*/
int main(int argc, char *argv[]){
    ...
    char filename[20];
    /* nakopirovanie hodnoty z argv[1] do filename */
    strcpy(filename, argv[1]);
    FILE *fp = NULL;
    /* otvorenie suboru v 'read' mode */
    fp = fopen(filename, "r");
    int height, width;
    /* nacitanie hodnot v danom formate ("%d %d ") zo suboru */
    fscanf(fp, "%d %d ", &width, &height);
    char world[height][width+1];
    for(int i=0; i<height; i++){
        /* nacitanie retazca (string) zo suboru */
        fgets(world[i], width+1, fp);
        /* nacitanie jedneho znaku */        
        fgetc(fp);
    }
